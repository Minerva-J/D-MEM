...
def error(preds, targets):
    ...
    # print('0',np.max(preds), np.max(targets))
    assert preds.size() == targets.size()
    bs,h,w = preds.size()
    n_pixels = float(bs*h*w)
    incorrect = float(preds.ne(targets).cpu().sum())#for python3 float/int = float
    # print('1',incorrect,n_pixels)
...
def train(model, trn_loader, optimizer, criterion, epoch):
    ...
    for idx, data in enumerate(trn_loader):
        ...
        # print('targets.shape',targets.shape)#torch.Size([1, 256, 256])
        # print('targets.shape',np.max(targets.cpu().numpy()))
        optimizer.zero_grad()
        output = model(inputs)
        ...
        nameP = './data/pred/Full/' + str(i) + '_P.png'
        nameT = './data/pred/Full/' + str(i) + '_T.png'
        # print(nameP,np.max(np.transpose(np.asarray(pred),(1,2,0))))
        # print(nameT,np.max(np.transpose(np.asarray(targets.cpu()),(1,2,0))))
        # cv2.imwrite(nameP,80*np.transpose(np.asarray(pred),(1,2,0)))
        # cv2.imwrite(nameT,80*np.transpose(np.asarray(targets.cpu()),(1,2,0)))
        # i = i + 1
